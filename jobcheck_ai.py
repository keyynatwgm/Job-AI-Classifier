# ------------------------------------------------------------
# PROGRAM TUGAS AKHIR S-1 TEKNOLOGI INFORMASI USU
# Nama                  : Kezia Natalia
# NIM                   : 211402002
# Dosen Pembimbing 1    : Ibu Ade Sarah Huzaifah S.Kom, M.Kom
# Dosen Pembimbing 2    : Bapak Dedy Arisandi S.T, M.Kom
# -------------------------------------------------------------
...
<style>
    ...
    /* Style Tambahan untuk Visualisasi Context Analysis */
    .context-box-digital {
...
# Fungsi klasifikasi kalimat berbasis konteks tugas untuk keperluan visualisasi XAI, memisahkan kalimat yang bermuatan digital/rutin dengan interaksi interpersonal (human touch).
def analyze_sentence_context(tasks):
    digital_kw = ["data", "analisis", "sistem", "model", "komputer", "perangkat", "mesin", "pengujian", "uji", "rancang", "kembang", "laporan", "prediksi", "evaluasi", "monitoring", "otomatis", "digital", "pencatatan", "verifikasi", "efisiensi", "akurasi", "teknologi", "informasi", "prosedur", "metode", "spesifikasi"]
    human_kw = ["mengajar", "guru", "siswa", "pasien", "merawat", "konseling", "empati", "didik", "sosial", "komunikasi", "negosiasi", "pimpinan", "rapat", "diskusi", "klien", "layanan"]
    ...
    for s in sentences:
        s_lower = s.lower()
        has_digital = any(re.search(r'\b' + w, s_lower) for w in digital_kw)
        has_human = any(re.search(r'\b' + w, s_lower) for w in human_kw)
        
        if has_digital and not has_human:
            analyzed_sentences.append((s, "digital"))
        elif has_human:
            analyzed_sentences.append((s, "human"))
        else:
            analyzed_sentences.append((s, "neutral"))
...
if page == "Overview":
    try:
        ...
        with st.container(border=True):
            ...
            if original_search_query and df_display.empty:
                st.warning(f"⚠️ Pekerjaan '{original_search_query}' tidak ditemukan. Silahkan cek di halaman **Cek Pekerjaan Teks** untuk menganalisis pekerjaan baru.")
            else:
                ...
                st.dataframe(paginate_dataframe(df_final, 15, "ov_page"), use_container_width=True, hide_index=True)
        ...
        with ch1:
            with st.container(border=True):
                ...
                fig_pie = px.pie(lc, names='Status Otomasi', values='Jumlah', hole=0.7, color_discrete_sequence=["#4F6DFF", "#E0E7FF"])
        ...
        with ch2:
            with st.container(border=True):
                ...
                fig_bar = px.bar(top10, x='exposure_score', y='title', orientation='h', labels={'exposure_score': 'Skor OES', 'title': 'Pekerjaan'}, color_discrete_sequence=["#4F6DFF"])
...
# --- PAGE: CEK TEKS ---
    ...
    with c2:
        ...
        job_desc = st.text_area("Deskripsi Task", placeholder="Contoh: Menginput data transaksi harian...", height=150, key="input_job_desc")

        # Reset hasil jika user mengetik ulang (lebih stabil daripada on_change)
        if "prev_desc" not in st.session_state:
            st.session_state.prev_desc = ""

        if job_desc != st.session_state.prev_desc:
            st.session_state.text_check_result = None
            st.session_state.prev_desc = job_desc

        # REAL-TIME CHARACTER COUNT
        char_count = len(job_desc) if job_desc else 0

        if char_count == 0:
            st.info("Jumlah Karakter: 0/700")
            job_desc_processed = ""
            ...
            st.success(f"Jumlah Karakter: {char_count}/700")
        ...
        if st.session_state.text_check_result:
            ...
            col_res1.metric("Hasil Klasifikasi", res["label"].title())
            col_res2.metric("Confidence Score", f"{res['conf']:.2%}")
            
            if res["conf"] >= THRESHOLD: 
                st.success("Pekerjaan ini berpotensi Terotomatisasi AI")
            else: 
                st.info("Pekerjaan ini relatif aman dari otomatisasi AI")
            
            # --- PENJELASAN AKADEMIS / EXPLAINABLE AI (XAI) UNTUK PENGGUNA ---
            with st.expander("🔍 Bagaimana Nilai Confidence Score Dihitung?", expanded=True):
                st.write(f"""
                1. **Vektor Konseptual**: Teks gabungan dari nama pekerjaan (*{job_name}*) dan deskripsinya ditransformasikan menjadi representasi numerik (*Dense Vector* 768 dimensi) menggunakan token khusus `[CLS]` dari model bahasa **IndoBERT**.
                2. **Ekstraksi Semantik**: Model IndoBERT membaca **konteks kalimat secara utuh**, bukan sekadar mencocokkan kata demi kata (*keyword matching*).
                3. **Klasifikasi Akhir**: Representasi tersebut diteruskan menuju jaringan saraf tiruan *Multi-Layer Perceptron* (MLP) yang menghasilkan distribusi probabilitas final sebesar **{res['conf']:.2%}**. 
                """)

            st.subheader("Analisis Tasks")
            st.caption("🔍 Analisis Kalimat: Memahami fokus dan karakteristik setiap tugas dalam deskripsi pekerjaan")
                        
            sentences_analysis = analyze_sentence_context(res["desc"])
            for sent, category in sentences_analysis:
                if category == "digital":
                    st.markdown(f"""<div class='context-box-digital'><b>🤖 Fokus Otomatisasi & Sistem (Digital):</b><br>"{sent}"</div>""", unsafe_allow_html=True)
                elif category == "human":
                    st.markdown(f"""<div class='context-box-human'><b>🤝 Fokus Interaksi & Hubungan Manusia (Human Touch):</b><br>"{sent}"</div>""", unsafe_allow_html=True)
                else:
                    st.markdown(f"""<div class='context-box-neutral'><b>📋 Tugas Rutin & Operasional Harian:</b><br>"{sent}"</div>""", unsafe_allow_html=True)
...
# --- PAGE: TESTING FILE ---
    ...
    uploaded_file = st.file_uploader("Upload CSV Data Testing", type=["csv"], key="file_checker", on_change=lambda: st.session_state.update({"test_result": None, "y_pred": None}))
    ...
    if uploaded_file:
        ...
        if not col_title or not col_tasks:
            ...
            st.success(f"✅ Kolom terdeteksi: '{col_title}' sebagai Nama Pekerjaan & '{col_tasks}' sebagai Deskripsi Tugas Pekerjaan")
    ...
    if st.session_state.test_result is not None and st.session_state.y_pred is not None:
        ...
        with col2:
            ...
            fig_bar = px.bar(top_df, x='confidence_numeric', y='Nama Pekerjaan', orientation='h', labels={'confidence_numeric': 'Confidence Score', 'Nama Pekerjaan': 'Pekerjaan'}, color_discrete_sequence=["#4F6DFF"])
            fig_bar.update_layout(yaxis={'categoryorder':'total ascending'})
            st.plotly_chart(fig_bar, use_container_width=True)
